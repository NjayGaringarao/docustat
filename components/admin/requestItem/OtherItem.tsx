...
const OtherItem: React.FC<OtherItemProps> = ({ request, onPress }) => {
  return (
    <TouchableOpacity
      onPress={onPress}
      className={`bg-background my-2 rounded-lg shadow-lg overflow-hidden border ${
        request.status == "success" ? "border-success" : "border-failed"
      }`}
    >
      {/* Header Section */}
      <View
        className={`flex-row justify-between items-center mb-2 ${
          request.status == "success" ? "bg-success" : "bg-failed"
        } px-4 py-2`}
      >
      ...
      <View className="px-4">
        ...
        <View className="mb-2">
          ...
          <Text className="text-uGray text-sm">Remarks : </Text>
          <View className="flex-row items-start justify-start gap-2 px-4">
